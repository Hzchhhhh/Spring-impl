...
import com.spring.aop.AnnotationAwareAspectJAutoProxyCreator;
import com.spring.aop.proxy.LazyInjectTargetSource;
 ...
 */
public class HzcApplicationContext {
    ...
    public HzcApplicationContext(Class configClass) {
    ...
    private Object createBean(String beanName, BeanDefinition beanDefinition) {
        // 相较于源码中的 createBean() 方法，这里省去了对 AOP 代理时机的判断，即用户是否自定义了 TargetSource
            ...
            if (beanDefinition.isSingleton()) {
                ...
                this.singletonFactories.put(beanName, new ObjectFactory<Object>() {
                    ...
                    public Object getObject() throws RuntimeException {
                        ...
                        for (BeanPostProcessor beanPostProcessor : HzcApplicationContext.this.beanPostProcessorList) {
                            if (beanPostProcessor instanceof SmartInstantiationAwareBeanPostProcessor) {
                                // getEarlyBeanReference 操作是BeanPostProcess的一种，也就是给子类重写的一个后处理器
                                // 目的是用于被提前引用时进行拓展
                                // 即：曝光的时候并不调用该后置处理器，只有曝光，且被提前引用的时候才调用
                                // 确保了被提前引用这个时机触发。
    ...
    /**
     * 尝试依次从 3 处缓存中取，这也是Spring解决循环依赖的开始
     *
     * @param beanName
    ...
    public List<Class<?>> getAllBeanClass() {
        return beanDefinitionMap.values().stream().map((Function<BeanDefinition, Class<?>>) BeanDefinition::getType).collect(toList());
    }
